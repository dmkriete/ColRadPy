...
The fractional abundance gives the charge state distribution of an ion species
...
Transport of ions through a plasma with spatially
varying temperature/density can cause the local fractional abundance to deviate
substantially from the expected steady-state distribution. To model the effect
of transport, this calculation allows the user to specify a 'dwell time'
characterizing the strength of transport along with a source of ions for each
charge/metastable state. A smaller dwell time corresponds to stronger transport.
To model a situation where ions are moving from regions to lower to higher
temperature (e.g. influx of sputtered neutral impurities), the source
distribution should be weighted toward lower charge states. To model the
opposite (e.g. outflux of fully ionized impurities from the confined plasma to
the scrape-off layer), the source distribution should be weighted toward higher
charge states.
"""
